...
fn load_file(loader: &dyn Loader, path: &Path, base: &Path, parent_env: &EnvVars, visited: &mut Vec<PathBuf>) -> Result<Config> {
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "schema") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::SchemaSpec> {
            let if_not_exists = get_attr_bool(b, "if_not_exists", env)?.unwrap_or(true);
            let authorization = get_attr_string(b, "authorization", env)?;
            Ok(crate::model::SchemaSpec { name: name.to_string(), if_not_exists, authorization })
        };
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "table") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::TableSpec> {
            let schema = get_attr_string(b, "schema", env)?;
            let if_not_exists = get_attr_bool(b, "if_not_exists", env)?.unwrap_or(true);

            // columns
            let mut columns: Vec<crate::model::ColumnSpec> = Vec::new();
            for cblk in b.blocks().filter(|bb| bb.identifier() == "column") {
                ...
                columns.push(crate::model::ColumnSpec { name: cname, r#type: ctype, nullable, default });
            }

            // primary_key
            let mut primary_key: Option<crate::model::PrimaryKeySpec> = None;
            for pkblk in b.blocks().filter(|bb| bb.identifier() == "primary_key") {
                ...
                primary_key = Some(crate::model::PrimaryKeySpec { name, columns: cols });
            }

            // indexes (including unique blocks)
            let mut indexes: Vec<crate::model::IndexSpec> = Vec::new();
            for iblk in b.blocks().filter(|bb| bb.identifier() == "index") {
                ...
                indexes.push(crate::model::IndexSpec { name: name_attr, columns: cols, unique });
            }
            for ublk in b.blocks().filter(|bb| bb.identifier() == "unique") {
                ...
                indexes.push(crate::model::IndexSpec { name: name_attr, columns: cols, unique: true });
            }

            // foreign keys
            let mut fks: Vec<crate::model::ForeignKeySpec> = Vec::new();
            for fkblk in b.blocks().filter(|bb| bb.identifier() == "foreign_key") {
                ...
                fks.push(crate::model::ForeignKeySpec { name, columns, ref_schema, ref_table, ref_columns, on_delete, on_update });
            }

            Ok(crate::model::TableSpec { name: name.to_string(), schema, if_not_exists, columns, primary_key, indexes, foreign_keys: fks })
        };
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "view") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::ViewSpec> {
            let schema = get_attr_string(b, "schema", env)?;
            let replace = get_attr_bool(b, "replace", env)?.unwrap_or(true);
            let sql = get_attr_string(b, "sql", env)?.context("view 'sql' is required")?;
            Ok(crate::model::ViewSpec { name: name.to_string(), schema, replace, sql })
        };
        if let Some(fe) = find_attr(b, "for_each") {
            let coll = expr_to_value(fe.expr(), &env)?;
            for_each_iter(&coll, &mut |k, v| {
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "materialized") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::MaterializedViewSpec> {
            let schema = get_attr_string(b, "schema", env)?;
            ...
            Ok(crate::model::MaterializedViewSpec { name: name.to_string(), schema, with_data, sql })
        };
        if let Some(fe) = find_attr(b, "for_each") {
            let coll = expr_to_value(fe.expr(), &env)?;
            for_each_iter(&coll, &mut |k, v| {
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "policy") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::PolicySpec> {
            let schema = get_attr_string(b, "schema", env)?;
            ...
            Ok(crate::model::PolicySpec { name: name.to_string(), schema, table, command, r#as: as_kind, roles, using, check })
        };
        if let Some(fe) = find_attr(b, "for_each") {
            let coll = expr_to_value(fe.expr(), &env)?;
            for_each_iter(&coll, &mut |k, v| {
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "function") {
        ...
        if let Some(fe) = find_attr(b, "for_each") {
            ...
            for_each_iter(&coll, &mut |k, v| {
                ...
                cfg.functions.push(crate::model::FunctionSpec {
                    name: name.clone(),
                    schema,
                    language,
                    returns,
            ...
            match coll {
                hcl::Value::Array(_) | hcl::Value::Object(_) => {
                    for_each_iter(&coll, &mut |k, v| {
                        ...
                        cfg.triggers.push(crate::model::TriggerSpec {
                            name: name.clone(),
                            schema,
                            table,
                            timing,
                ...
                _ => {
                    ...
                    let level = expr_to_string(fe.expr(), &env)?;
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "extension") {
        ...
        if let Some(fe) = find_attr(b, "for_each") {
            ...
            for_each_iter(&coll, &mut |k, v| {
                ...
                cfg.extensions.push(crate::model::ExtensionSpec { name: name.clone(), if_not_exists, schema, version });
                Ok(())
            })?;
        } else {
            let if_not_exists = get_attr_bool(b, "if_not_exists", &env)?.unwrap_or(true);
            ...
            cfg.extensions.push(crate::model::ExtensionSpec { name, if_not_exists, schema, version });
        }
    }
    ...
    for blk in body.blocks().filter(|b| b.identifier() == "enum") {
        let name = blk
            ...
            .to_string();
        let b = blk.body();
        let parse_one = |name: &str, b: &hcl::Body, env: &EnvVars| -> Result<crate::model::EnumSpec> {
            let schema = get_attr_string(b, "schema", env)?;
            let values = match find_attr(b, "values") {
                Some(attr) => expr_to_string_vec(attr.expr(), env)?,
                None => bail!("enum '{}' requires values = [..]", name),
            };
            Ok(crate::model::EnumSpec { name: name.to_string(), schema, values })
        };
        if let Some(fe) = find_attr(b, "for_each") {
            let coll = expr_to_value(fe.expr(), &env)?;
            for_each_iter(&coll, &mut |k, v| {
